...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx, grads["W1"], grads["b1"] = affine_relu_backward(dx1, cache1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            if self.use_batchnorm:
                out, cache = affine_batchnorm_relu_forward(acti,
                ...
                out, cache = affine_relu_dropout_forward(acti,
                                                          self.params['W'+str(layer)],
                                                          self.params['b'+str(layer)],
                                                          self.dropout_param)
            else:
                out, cache = affine_relu_forward(acti,
                                                  self.params['W'+str(layer)],
                                                  self.params['b'+str(layer)])
            acti = out
            caches.append(cache)
        scores, cache = affine_forward(acti,
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        ############################################################################
        loss, xn = softmax_loss(scores, y)
        l2 = 0
        for layer in range(self.num_layers):
            l2 += np.sum(self.params['W' + str(layer+1)] * self.params['W' + str(layer+1)])
        loss += self.reg * 0.5 * l2

        dx, dw, db = affine_backward(xn, caches[self.num_layers - 1])
        ...
        for layer in range(self.num_layers - 2, -1, -1):
            if self.use_batchnorm:
                dx, dw, db, dga, dbe = affine_batchnorm_relu_backward(dx, caches[layer])
                grads["gamma" + str(layer + 1)] = dga
                grads["beta" + str(layer + 1)] = dbe
            elif self.use_dropout:
                dx, dw, db = affine_relu_dropout_backward(dx, caches[layer])
            else:
                dx, dw, db = affine_relu_backward(dx, caches[layer])
...
def affine_batchnorm_relu_forward(x, W, b, gamma, beta, param):
    af_x, af_cache = affine_forward(x, W, b)
    bn_x, bn_cache = batchnorm_forward(af_x, gamma, beta, param)
    out, rl_cache = relu_forward(bn_x)
    cache = [af_cache, bn_cache, rl_cache]
    return out, cache

def affine_batchnorm_relu_backward(dout, cache):
    af_cache, bn_cache, rl_cache = cache
    dx = relu_backward(dout, rl_cache)
    dx, dga, dbe = batchnorm_backward(dx, bn_cache)
    dx, dw, db = affine_backward(dx, af_cache)
    return dx, dw, db, dga, dbe

def affine_relu_dropout_forward(x, W, b, param):
    af_x, af_cache = affine_forward(x, W, b)
    relu_x, rl_cache = relu_forward(af_x)
    out, drop_cache = dropout_forward(relu_x, param)
    cache = [af_cache, rl_cache, drop_cache]
    return out, cache

def affine_relu_dropout_backward(dout, cache):
    af_cache, rl_cache, drop_cache = cache
    dx = dropout_backward(dout, drop_cache)
    dx = relu_backward(dx, rl_cache)
    dx, dw, db = affine_backward(dx, af_cache)
    return dx, dw, db